...
<body>
    <div class="container">
        <h1 style="font-size:20pt">CRUD JS</h1>

        <h3>Data Keluarga</h3>
        <br />
        <button class="btn btn-success" onclick="add_master()"><i class="glyphicon glyphicon-plus"></i> Tambah Keluarga Baru</button>
...
function edit_master(id)
{
    ...
    $.ajax({
        url : "<?php echo site_url('C_master/master_edit/')?>/" + id,
        type: "GET",
        dataType: "JSON",
...
function save_unit()
{
    ...
    if(save_method == 'add_user') {
        url = "<?php echo site_url('C_master/master_add')?>";
    } else {
        url = "<?php echo site_url('C_master/master_update')?>";
    }
...
function delete_master(id)
{
    if(confirm('Apakah kamu yakin akan menghapus data ini?'))
    {
        ...
        $.ajax({
            url : "<?php echo site_url('C_master/master_delete')?>/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
            ...
            <div class="modal-body form">
                <form action="#" id="form" class="form-horizontal">
                    ...
                    <div class="form-body">
                        ...
                        <div class="form-group">
                            <label class="control-label col-md-3">Anak dari</label>
                            <div class="col-md-9">
                                <select name="anak" class="form-control">
                                    <option value="">--Pilih--</option>
                                    <?php
                                    $query = $this->db->query('SELECT * FROM silsilahkeluarga');
                        ...
                        <div class="form-group">
                            <label class="control-label col-md-3">Cucu dari</label>
                            <div class="col-md-9">
                                <select name="cucu" class="form-control">
                                    <option value="">--Pilih--</option>
                                    <?php
                                    $query = $this->db->query('SELECT * FROM silsilahkeluarga');
                                    foreach ($query->result() as $row)
